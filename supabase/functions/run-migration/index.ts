...
Deno.serve(async (req) => {
  ...
  const migrations = [
    ...
    `DO $$ BEGIN
       IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='organization_usage' AND policyname='org_isolation_usage') THEN
    ...
    // Phase 2: upsert_org_usage RPC function
    `CREATE OR REPLACE FUNCTION public.upsert_org_usage(
       ...
       p_proposals_sent INTEGER DEFAULT 0
     ...
     BEGIN
       INSERT INTO public.organization_usage(
         ...
         p_projects_created, p_proposals_sent, NOW()
       )
    ...
    // Phase 3: onboarding_completed column
    `ALTER TABLE public.organizations ADD COLUMN IF NOT EXISTS onboarding_completed BOOLEAN NOT NULL DEFAULT FALSE`,

    // Phase 3: sync profile -> org on insert trigger
    `CREATE OR REPLACE FUNCTION public.sync_profile_to_org()
     RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
     BEGIN
       IF NEW.organization_id IS NOT NULL AND (NEW.company_name IS NOT NULL OR NEW.logo_url IS NOT NULL) THEN
    ...
    `CREATE TRIGGER trg_sync_profile_to_org
       AFTER INSERT OR UPDATE OF company_name, logo_url ON public.profiles
       FOR EACH ROW EXECUTE FUNCTION public.sync_profile_to_org()`,

    // Phase 3: indexes for performance
    `CREATE INDEX IF NOT EXISTS idx_organization_usage_org_period ON public.organization_usage(organization_id, period_start DESC)`,
  ...
  for (const sql of migrations) {
    const { error } = await supabase.rpc('exec_raw', { sql })
      .catch(() => ({ error: null }))
